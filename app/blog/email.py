...
class Email:
    ...
    def generate_new_post_message(self) -> str:
        ...
        <a href='{reverse('blogs:detail', args=[self.post.blog.id])}'>Link</a>
        """
        return html
    ...
    def generate_update_post_message(self) -> str:
        html = f"""Update post from {self.post.blog.user.username} published at link 
                <a href='{reverse('blogs:post', args=[self.post.id])}'>Link</a>
                """
        return html

    def send_update_post_message(self, to: list):
        email_message = EmailMessage("Update post", self.generate_new_post_message(),
                                     settings.EMAIL_HOST_USER, to)
        email_message.content_subtype = 'html'
        email_message.send()
